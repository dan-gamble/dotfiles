...
export function ProductGroupItemsCard(props: ProductGroupItemsCardProps) {
	...
	const selectItems = useCallback(async () => {
		...
		const newData = selected
			.map((selectedElement) => {
				if (!("variants" in selectedElement)) return null;

				const variantId = selectedElement.variants[0]
				...
				return {
					id,
					productId: selectedElement.id as ShopifyGid<"Product">,
					variantId,
		...
		for (const existingItem of existingItems) {
			await productGroupItems.shift();
		}

		for (const newDatum of newData) {
			await productGroupItems.push(newDatum);
		}
