...
function ProductGroupItemsTableRow<Type extends ProductGroupItemV>({
	...
	const shopifyProduct = getProductGroupItems(productGroupId).find(
	...
	}, [fetcher.load, hasFetched, productGroupId, shopifyProduct]);
				...
				<IndexTable.Cell>
					<InlineStack gap="400" blockAlign="center" wrap={false}>
						<div style={{ flexShrink: 0 }}>
							<SkeletonThumbnail size={thumbnailSize} />
						</div>
	...
	const selectedProduct =
		shopifyProduct ??
		(fetcher.data?.selectedItems ?? []).find(
			(shopifyItem) => shopifyItem.productId === item.productId,
		);
