...
import { priceRowsToValidatorStructure } from "./Price.server";
...
export async function createGlobalProductGroup({
	...
	const batchResponse = await db.batch([
		...
		...Object.values(data.productGroup.prices).flatMap((prices) => {
			return prices.map((price) =>
				db.insert(prices).values({
					id: prices.id,
					amount: prices.amount,
					currencyCode: prices.currencyCode as CurrencyCode,
		...
		...data.productGroup.productGroupItems.flatMap((item) => {
			return [
				...
				...Object.values(item.prices).map((price) => {
					return db.insert(prices).values({
						id: price.id,
						amount: price.amount,
...
export async function updateGlobalProductGroup({
	...
	const operations: Parameters<typeof db.batch>[0] = [
		...
		...Object.values(data.productGroup.prices).map((price) => {
			return db
				.insert(prices)
				.values({
					id: price.id,
