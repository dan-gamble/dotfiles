...
export function AddProductGroupPopover({
	...
	const globalProductGroupOptions: SelectOption[] =
		madeToOrderState.globalProductGroups
			.filter((globalProductGroup) => {
				const currentProductGroups = form.transient.value("productGroups");

				return !currentProductGroups.some(
					(productGroup) =>
						productGroup.id === globalProductGroup.productGroup.id,
				);
			})
			.map((globalProductGroup) => {
				return {
	...
	console.log({ globalProductGroupOptions });
