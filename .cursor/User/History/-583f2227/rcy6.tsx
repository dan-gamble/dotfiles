...
import { clientQuery } from "../../product-groups/$id.selected-items";
...
export function AddProductGroupPopover({
	...
	const { data } = useQuery({
		...
		queryFn: async () => {
			const response = await fetch(
				`/app/products/${globalProductGroupId}/selected-items`,
			);
			...
			return (await response.json()) as Awaited<
				ReturnType<import("../../product-groups/$id.selected-items").loader>
			>;
		},
	});
	...
	const addProductGroup = useCallback(async () => {
		...
		if (globalProductGroupId !== "") {
			await fetch(`/app/product-groups/${productGroup.id}/selected-items`)
				.then((data) => {
					console.log({ data });
				})
				.catch((error) => {
