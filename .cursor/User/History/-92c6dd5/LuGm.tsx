...
const MadeToOrderContext = createContext<
	| {
			...
			isGlobalProductGroup: (id: string) => boolean;
	  }
	| undefined
>(undefined);
...
export function MadeToOrderProvider({
	...
	const isGlobalProductGroup = useCallback(
		(id: string) => {
			return state.globalProductGroups.some((globalProductGroup) => {
				console.log({ id, globalProductGroup });

				return globalProductGroup.productGroup.id === id;
			});
		},
		[state.globalProductGroups],
	);
